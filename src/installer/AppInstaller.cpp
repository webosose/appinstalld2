...
AppInstaller::AppInstaller()
    : m_installerDataPath(Settings::instance().m_installerDataPath)
{
...
std::shared_ptr<Task> AppInstaller::install(const std::string& appId, const std::string& ipkUrl,
    pbnjson::JValue appInfo,
    int &errorCode, std::string &errorText,
    bool verify, bool allowDowngrade, std::string taskName)
{
...
std::shared_ptr<Task> AppInstaller::remove(const std::string& appId, pbnjson::JValue appInfo,
    int& errorCode, std::string &errorText,
    bool verify)
{
...
void AppInstaller::onUpdateTask(const Task &task)
{
    ...
    {
        LOG_WARNING(MSGID_REPLY_SUBSCR_FAIL,2,PMLOGKS(KEY,key.c_str()),
                                                    PMLOGKS(PAYLOAD,payload.c_str()),"reply subscription failed");
...
std::shared_ptr<Task> AppInstaller::createTask(const std::string &id, const std::string &name, pbnjson::JValue param)
{
    ...
    if(name == "InstallTask") task->prepareStep(StepSettings::instance().m_mapInstallSteps);
    else if(name == "RemoveTask") task->prepareStep(StepSettings::instance().m_mapRemoveSteps);
    else return nullptr;
...
bool AppInstaller::contains(Task *task)
{
    ...
    auto it = std::find_if(m_mapTask.begin(), m_mapTask.end(),
...
pbnjson::JValue AppInstaller::toJValue() const
{
    ...
    std::for_each(m_mapTask.begin(), m_mapTask.end(),
